...
def test_initiation_verify_count_1_is_three_stages():
    stages = plan_stages(
        "initiation", lead="gpt", contributors=["gemini"], verify_count=1, max_attempts=3
    )
...
def test_deep_review_is_author_then_verifier():
    stages = plan_stages("deep_review", lead="gpt", contributors=["gemini"], max_attempts=3)
...
def test_distillation_is_the_meta_house_only():
    stages = plan_stages("distillation", meta_house="claude", max_attempts=3)
...
def test_plan_stages_threads_max_attempts_into_every_stage():
    # SPEC §4.8: max_attempts is from config at creation, never a silent default.
    stages = plan_stages(
        "initiation",
        lead="gpt",
        contributors=["gemini", "deepseek"],
    ...
    assert all(s.max_attempts == 5 for s in stages)
