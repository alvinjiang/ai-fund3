...
## 2026-07-18 — BUGS.md review fixes (spec-domain/spec-core hardening)

Acted on every numbered finding in `BUGS.md` (the 2026-07-18 review of the merged
spec-domain/spec-core). 343 unit + 14 integration green; ruff + format clean.

- **#1 coverage-lock race** — `runs_repo.start_run` now uses dialect-aware
  `INSERT ... ON CONFLICT DO NOTHING (coverage_id)` + re-read for ownership; the loser
  returns False, never raises. Barrier-synchronized integration test (8 threads) proves
  exactly one acquires, seven refuse, zero IntegrityError.
- **#2 prediction idempotency race** — `register_from_stage` uses
  `ON CONFLICT DO NOTHING (idempotency_key)` + re-select; concurrent-replay integration
  test registers exactly one row.
- **#3 retry backoff** — `core/domain/backoff.py` (`BackoffPolicy` + `backoff_seconds`,
  exponential, caller-injected jitter); `queue.fail_stage`/`reap_expired` consume the
  `set_available_at_backoff` side effect and apply it. (This was pre-existing in the
  working tree; adopted as-is — better-designed than my draft.)
- **#4 FK CASCADE→RESTRICT** — `coverage_contributors`/`levels`/`transitions`/
  `lead_history` now `ondelete=RESTRICT` (the audit ledger is protected); integration
  test proves coverage delete is blocked while the sanctioned run→stages CASCADE works.
- **#5 tautological alembic baseline** — regenerated `0001_baseline.py` as real
  autogenerate DDL (69 op.create_table/index/constraint incl. partial indexes + the
  use_alter doctrine↔runs FK). **Verified `alembic check` now detects drift**: a
  throwaway column on `coverage` → `FAILED: add_column`; revert → clean.
- **#6 claim FOR UPDATE scoped** — `with_for_update(skip_locked=True, of=RunStage)` so
  the lock does not extend to the joined `runs` row.
- **#7 set_lead checks `enabled`** — `TransitionContext.new_house_enabled` + guard +
  repo passes `h.enabled`; test that a disabled house is rejected.
- **#8 `price_pins` table** — added (SPEC-MARKETDATA §2.1), folded into the regenerated
  baseline.
- **#9 legal-transition tests** — `LEGAL` now carries the expected side-effect set;
  `test_legal_transition` asserts set equality (a dropped/typo'd effect fails).
- **Minors** — `fail_stage` forwards `attempt.kill_reason`; removed unused
  `RunAction.RESUME` (no `waiting_pm→running` edge in §6.1); outbox docstring corrected +
  `pending_count`→`pending_exists`; `runs.mutates_dossier`/`disagreements.material` got
  `server_default`; planner threads `policy.max_attempts` into every `StageIn`; dead
  `acquire_lock`/`release_lock` branches removed from `coverage.py`; added
  `core/db/repo/dossier.py` (§2 layout); README Layout updated; migrations excluded from
  ruff (autogenerated DDL).
- **Untested paths** — `requeue_run` test (lock survives `running→queued`); reaper
  `kill_reason='lease_expired'` assertion; `monitor_tick` starts against a held lock.
- **CI** — `.github/workflows/ci.yml`: ruff + unit + the no-hardcoded-models guard on
  every push/PR; integration (Docker Postgres) as a separate job. Finds #1/#2/#5/#6
  would recur silently without it.

**Deferred (still open):** the §9.2 transient `running→queued` re-entry at the *engine*
level (retry is absorbed in `FakeRunner.drain`); per-house **daily** budgets
(`house_budget_days`); the SQLite Decimal-fidelity `TypeDecorator` (production Postgres
NUMERIC is unaffected — it is a unit-test-fidelity gap, flagged "consider" in BUGS.md).

## 2026-07-18 — BUGS.md round 2 (review-fixes-2 branch, NOT merged — pending review)

The remaining open items from the round-2 `BUGS.md`. On branch `review-fixes-2`, left
unmerged because item 1 touches `core/orchestrator/` (spec-core territory). 344 unit +
15 integration green; ruff + format clean.

- **#1 `max_attempts` silent default duplicated, not removed.** Dropped the default from
  `StageIn.max_attempts` and `plan_stages(max_attempts=...)` (both required now), and from
  `RunPolicy.max_attempts` (config must state it — AGENTS.md no-silent-fallback). The live
  literal at `engine.py`'s auto-cross-check append now reads `fund.policy(run.type)
  .max_attempts` like every other stage, so a config change applies uniformly. Every test
  `StageIn(...)` / `plan_stages(...)` / `RunPolicy(...)` states it explicitly (a script
  patched the call sites; `test_planner` gained a threading assertion). Verify:
  `grep -rn "max_attempts.*=.*3" core/` → no defaults or literals, only config reads.
- **#2 `alembic check` drift-detection guard.** New integration test injects a canary
  column onto `Base.metadata.tables["runs"]` at runtime and asserts `command.check`
  raises `AutogenerateDiffsDetected`; teardown removes it in a `finally` (the metadata is
  process-global — unconditional restore verified by running the full integration suite
  after, which stays green).
- **#3 `mutates_dossier` default — kept `true`, confirmed unreachable.** The server
  default is the fail-safe (wrong-true takes an unnecessary lock; wrong-false breaks
  serialization). Added the assertion to the monitor-vs-held-lock test: a `monitor_tick`
  run has `mutates_dossier is False` (derived from run type in `create_run`, never the
  default) and starts without a lock while a mutating run holds one.
- **#4 CI unused Postgres service.** Dropped the `services: postgres` block + its
  `DATABASE_URL` from `.github/workflows/ci.yml` (the suite starts its own testcontainer);
